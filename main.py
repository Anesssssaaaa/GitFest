...
list_r=["Выберите язык","Кнопка","Русский","Англисский","Настройки"]
list_e=["Choose a language","Button","Rassian","English","Settings"]
list_t=["Выберите тему","Кнопка","Темная","Англисский","Настройки"]


def tema(): 
    fg = Tk()
    fg.title("Настройки")
    fg.geometry("300x200+100+100")

    L1=Label(fg,text="Выберите тему")
    def tem():
        L1.config(text=list_t[0])
        B1.config(text=list_t[2])
        B2.config(text=list_t[3])



    B1=Button(fg,text="Темная тема")
    B2=Button(fg,text="Светлая тема")
    L1=Label(fg, text="Выберите тему")
    ...
    L2=Label(fg,text="Выберите язык")
    def rus():
        L2.config(text=list_r[0])
        B3.config(text=list_r[2])
        B4.config(text=list_r[3])
        fg.title("настройки")
    def eng():
        L2.config(text=list_e[0])
        B3.config(text=list_e[2])
        B4.config(text=list_e[3])
        tk.title("Settings")
    
    B3=Radiobutton(fg,text="Русский",command=rus, value=1)
    B4=Radiobutton(fg,text="Англиский",command=eng, value=0)
...
M2.add_command(label="Настройки",command=tema)
